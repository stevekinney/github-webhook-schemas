...
describe('createGithubWebhookRouter', () => {
  describe('config-object style', () => {
    ...
    it('routes a payload to an action-specific handler', () => {
      const calls: string[] = [];
      const router = createGithubWebhookRouter({
      ...
      router(createPullRequestOpenedEvent());
      expect(calls).toEqual(['opened']);
    });

    it('fires action-specific handlers before base-event handlers', () => {
      const calls: string[] = [];
      const router = createGithubWebhookRouter({
        pullRequest: () => {
          calls.push('base');
    ...
    it('infers route handler payload types', () => {
      createGithubWebhookRouter({
        push: (event) => {
  ...
  describe('dynamic registration with .on()/.off()', () => {
    ...
    it('is chainable', () => {
      ...
      const result = router.on('push', handler).off('push', handler).on('push', handler);
  ...
  describe('mixed config + dynamic registration', () => {
    it('supports both initial handlers and .on() handlers', () => {
      const calls: string[] = [];

      const router = createGithubWebhookRouter({
        push: () => {
          calls.push('config-push');
        },
      });

      router.on('pullRequestOpened', () => {
        calls.push('dynamic-pr');
      });

      router(createPushEvent());
      router(createPullRequestOpenedEvent());
      expect(calls).toEqual(['config-push', 'dynamic-pr']);
    });
  });

  describe('async handlers', () => {
    it('handles async handlers (fire-and-forget)', async () => {
      const calls: number[] = [];
      const router = createGithubWebhookRouter();
  ...
  describe('error handling', () => {
    ...
    it('throws if handler errors and no error listener registered', () => {
      ...
      expect(() => router(createPushEvent())).toThrow('unhandled');
    });
  });

  describe('return type', () => {
    it('returns a WebhookRouter', () => {
      const router = createGithubWebhookRouter();
      expectTypeOf(router).toMatchTypeOf<WebhookRouter>();
    });

    it('is callable with void return', () => {
      const router = createGithubWebhookRouter();
      const result = router(createPushEvent());
